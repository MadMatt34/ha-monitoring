...
class HAMonitoringCoordinator(
    ...
    def __init__(
        ...
        self._scan_timestamp_listeners: list[
            Callable[[], None]
        ] = []
    ...
    async def _async_update_data(
        ...
        if self._cached_backup_info is None:
            ...
            self._last_backup_scan_time = (
                dt_util.utcnow()
            )

            self._backup_scan_time_cache[
                self.entry.entry_id
            ] = self._last_backup_scan_time
        ...
        if (
            ...
            self._cached_system_stats = (
                await async_get_system_stats(
                    self.hass,
                    self._ha_start_time,
                )
            )
