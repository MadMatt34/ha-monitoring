...
class HAMonitoringCoordinator(
    ...
    def __init__(
        ...
        self._ha_start_time: datetime | None = hass.data[
            DOMAIN
        ].get(_HA_START_TIME_KEY)
        ...
        self._cached_backup_info: MonitoringBackupData | None = (
            self._backup_cache.get(
                entry.entry_id
            )
        )

        self._backup_scan_time_cache: dict[str, datetime] = (
            hass.data[DOMAIN].setdefault(
        ...
        self._last_backup_scan_time: datetime | None = (
            self._backup_scan_time_cache.get(
                entry.entry_id
            )
        )
        ...
        try:
            ...
            _LOGGER.debug(
                "[HA Monitoring] Le composant Backup "
    ...
    async def async_set_maintenance_mode(
        ...
        stored_data = (
            await self._maintenance_store.async_load()
        ) or {}

        stored_data[self.entry.entry_id] = enabled

        await self._maintenance_store.async_save(
            stored_data
        )
    ...
    def async_add_scan_timestamp_listener(
        ...
        def _remove_listener() -> None:
            if listener in self._scan_timestamp_listeners:
                self._scan_timestamp_listeners.remove(
                    listener
                )
    ...
    def _async_update_scan_timestamp_listeners(
        ...
        for listener in tuple(
            self._scan_timestamp_listeners
        ):
            listener()
    ...
    def _async_backup_event(
        ...
        self._previous_backup_info = (
            self._cached_backup_info
        )
        self._last_backup_event = event
        self._last_backup_event_time = (
            dt_util.utcnow()
        )
    ...
    def _get_last_seen_suffixes(
        ...
        localized_suffix = (
            LOCALIZED_LAST_SEEN_SUFFIX.get(
    ...
    async def _async_get_common_translations(
        ...
        if (
            ...
            and self._cached_translation_language
            == language
        ):
            return (
    ...
    async def _async_update_data(
        ...
        if self._cached_backup_info is None:
            ...
            self._last_backup_scan_duration = time.monotonic() - scan_start
        ...
        if (
            ...
            self._last_system_info_scan_duration = time.monotonic() - scan_start
        ...
        unknown_platform, unknown_version = (
            await self._async_get_common_translations()
        )
        ...
        if (
            ...
            self._last_trace_scan_duration = time.monotonic() - scan_start
        ...
        integrations = (
            await async_get_failed_integrations(
                self.hass,
                options.get(
                    CONF_EXCLUDED_INTEGRATIONS,
                    [],
        ...
        self._last_scan_duration = time.monotonic() - scan_start
...
type HAMonitoringConfigEntry = ConfigEntry[
