...
def _unwrap_traces(val: Any) -> list[Any]:
    """Déballe récursivement les conteneurs (TraceBuckets, dicts, listes)."""
    if val is None:
        return []
...
def _get_raw_trace_timestamp(trace: Any) -> Any:
    ...
    if ts_attr:
        if isinstance(ts_attr, dict):
            val = ts_attr.get("start") or ts_attr.get("finish")
            if val:
                return val
        else:
            val = getattr(ts_attr, "start", None) or getattr(ts_attr, "finish", None)
            if val:
                return val
...
def _parse_timestamp(val: Any) -> datetime | None:
    ...
    if isinstance(val, str):
        ...
        if parsed:
            return parsed.replace(tzinfo=dt_util.UTC) if parsed.tzinfo is None else parsed
    return None
