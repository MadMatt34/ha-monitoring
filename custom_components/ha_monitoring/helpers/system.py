...
def _snapshot_states(
    ...
    # appartenant à HA Monitoring. Cela évite un async_get() par état.
    ...
    for state_obj in hass.states.async_all():
        ...
        is_last_seen = entity_id.endswith(last_seen_suffixes)
        ...
        # Les registry ne sont nécessaires que pour les entités last_seen.
        if is_last_seen:
    ...
    for entry in entries:
        ...
        if friendly_reason is None:
            state_issue_key = {
                ...
                ConfigEntryState.MIGRATION_ERROR: (f"component.{DOMAIN}.issues.setup_error.title"),
            }.get(entry.state)
